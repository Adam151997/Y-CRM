    ...
    execute: async ({ accountId, contractValue, contractStartDate, contractMonths = 12, csManagerId }) => {
      ...
      try {
        ...
        const result = await prisma.$transaction(async (tx) => {
          ...
          const tasks = [];
          const taskDefs = [
            { title: "Welcome call", days: 1, taskType: "CALL" as const },
            { title: "Product training session", days: 3, taskType: "MEETING" as const },
            { title: "Check-in on initial setup", days: 7, taskType: "FOLLOW_UP" as const },
            { title: "First month review", days: 30, taskType: "MEETING" as const },
          ];
